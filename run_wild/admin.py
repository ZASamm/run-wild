...
class QuestPostAdmin(SummernoteModelAdmin):
    ...
    def preview_map(self, obj):
        if obj.map_route and obj.map_route.url:
            return format_html('<img src="{}" style="max-height: 50px;"/>', obj.map_route.url)
        return "No map"
    preview_map.short_description = 'Route Map'

@admin.register(QuestRecord)
class QuestRecordAdmin(admin.ModelAdmin):
    list_display = ('quest', 'runner', 'completion_time', 'tokens_earned', 'approved')
    list_filter = ('approved', 'is_personal_best')
    search_fields = ['runner__username', 'quest__title']
    readonly_fields = ('tokens_earned', 'pace', 'is_personal_best')
